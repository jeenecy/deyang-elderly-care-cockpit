...
- **G03 老人档案管理**：老人档案 CRUD、健康标签多选、紧急联系人、按区县/状态筛选、批量操作、CSV 导出、详情抽屉
...
```
src/
  views/BigScreen.vue        # 驾驶舱大屏（2D/3D 地图 + 多板块）
  views/admin/               # 后台管理各页面（含 ElderManage.vue 老人档案）
  stores/                    # Pinia：data.js（数据层）/ sections.js（大屏板块）/ screen.js
  constants/dict.js          # 业务枚举中心化
  data/                      # 地图 GeoJSON / 行政区划
  utils/                     # 地图引擎 map3dThree.js / map3dEngine.js
server/                      # Node JSON 文件服务（records API）
scripts/                     # 数据生成脚本
```
